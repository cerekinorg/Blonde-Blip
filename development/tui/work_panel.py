...
class WorkPanel(Vertical):
    ...
    def __init__(self, **kwargs):
        ...
        self.saved_chat_messages = []  # Store chat messages when switching modes
    ...
    @on(DirectoryTree.FileSelected, "#chat_file_tree")
    def on_chat_file_selected(self, event: DirectoryTree.FileSelected):
        """Handle file selection in chat mode - open in editor"""
        path = Path(event.path)
        if path.is_file():
            # Switch to editor mode and load file
            self.current_mode = "editor"
            self._switch_to_editor_with_file(path)
            self.notify(f"Opened {path.name} in editor", severity="information")
    ...
    def watch_current_mode(self, old_mode: str, new_mode: str):
        ...
        if new_mode == "chat":
            ...
            # File tree should always be visible in editor mode
            # Show both file trees (chat and editor) in editor mode
            if self.file_tree_widget:
    ...
    def _switch_to_chat_with_file(self, file_path: Path):
        """Switch to editor mode and load file"""
        self.current_mode = "editor"
        self._switch_to_editor_with_file(file_path)
    
    def _switch_to_chat(self):
        """Switch to chat mode"""
        # Save current chat messages before removing chat view
        if self.chat_view and VIEWS_AVAILABLE:
            self.saved_chat_messages = self.chat_view.messages.copy()
        ...
        if self.chat_view and VIEWS_AVAILABLE and self.saved_chat_messages:
            for msg in self.saved_chat_messages:
        ...
        if self.app:
            try:
                ...
                if hasattr(blip_panel, 'set_editor_mode'):
    ...
    def _switch_to_editor_with_file(self, file_path: Path):
        """Switch to editor mode and load a file"""
        ...
        if self.editor_view and VIEWS_AVAILABLE:
            self.editor_view.load_file(file_path)
        
        # Switch to editor mode
        self.current_mode = "editor"
        
        self.border_title = f"Workspace (Editor - {file_path.name})"
        self.current_child = self.editor_view
